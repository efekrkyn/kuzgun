...
import { buildPhoneFootprint } from '@/lib/phone-footprint';
import { isRateLimited, getClientIp } from '@/lib/ssrf-guard';

const phoneUtil = PhoneNumberUtil.getInstance();

// google-libphonenumber line-type enum → human label
const LINE_TYPE: Record<number, string> = {
...
export async function GET(req: Request) {
  ...
  try {
      ...
      // Authoritative line-type from libphonenumber metadata.
      const lineType = LINE_TYPE[phoneUtil.getNumberType(parsedNumber)] || 'UNKNOWN';
      ...
      // phoneinfoga-style OSINT footprint dorks (keyless search links).
      const footprint = buildPhoneFootprint({
        e164: formatE164,
        intlNoPlus: formatE164.replace('+', ''),
        rawLocal: String(nationalNumber),
        national: formatNat,
      });
