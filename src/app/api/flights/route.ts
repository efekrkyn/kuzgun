...
const REGIONS = [
  ...
  { lat: 39.0, lon: 35.0, dist: 700 },      // Türkiye + Orta Doğu (yoğun kapsama)
  { lat: 41.0, lon: 29.0, dist: 350 },      // İstanbul / Marmara
];
...
function classifyFlight(f: any) {
  ...
  // ANKARA VIP / MILITARY DETECTION
  // Bounding box for Ankara Region: Lat 39.0 - 40.5, Lng 32.0 - 33.5
  let isAnkaraVip = false;
  if (lat >= 39.0 && lat <= 40.5 && lon >= 32.0 && lon <= 33.5) {
    if (category === 'jet' || category === 'military' || callsign.startsWith('TUR')) {
      isAnkaraVip = true;
    }
  }

  return {
    callsign: isAnkaraVip ? `[ANKARA RADAR] ${callsign}` : callsign,
