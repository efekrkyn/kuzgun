...
/**
 * KUZGU — Internet Outage Detection (IODA)
...
export async function GET() {
  try {
    ...
    const res = await fetch(url, {
      ...
      cache: 'no-store',
      headers: { 'User-Agent': 'KUZGU/4.2', 'Accept': 'application/json' },
    });

    console.log('[KUZGU] IODA response status:', res.status);
    ...
    console.error('[KUZGU] IODA fetch error:', error);
