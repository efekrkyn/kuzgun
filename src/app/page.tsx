...
const AiCommand = dynamic(() => import('@/components/AiCommand'), { ssr: false });
...
const UptimeClock = () => {
  ...
  return <span className="hidden lg:inline">ÇALIŞMA: <span className="text-[var(--gold-primary)]">{uptime}</span></span>;
};
...
export default function Dashboard() {
  ...
  // ── AI Map Control ──
  const handleMapCommand = useCallback((cmds: any[]) => {
    cmds.forEach(cmd => {
      if (cmd.lat !== undefined && cmd.lng !== undefined) {
        setFlyToLocation({ lat: cmd.lat, lng: cmd.lng, ts: Date.now() });
        if (cmd.zoom !== undefined) {
          setMapView(v => ({ ...v, zoom: cmd.zoom }));
        }
      }
      if (cmd.enable_layers) {
        const layers = cmd.enable_layers.split(',').map((s: string) => s.trim());
        setActiveLayers(prev => {
          const next = { ...prev };
          layers.forEach(l => { if (l in next) (next as any)[l] = true; });
          return next;
        });
      }
      if (cmd.disable_layers) {
        const layers = cmd.disable_layers.split(',').map((s: string) => s.trim());
        setActiveLayers(prev => {
          const next = { ...prev };
          layers.forEach(l => { if (l in next) (next as any)[l] = false; });
          return next;
        });
      }
    });
  ...
  const handleRightClick = useCallback(async (coords: { lat: number; lng: number }) => {
    ...
    } catch (e) { console.warn('[KUZGU] Suppressed error:', e instanceof Error ? e.message : e); } finally { setDossierLoading(false); }
  }, []);
  ...
  const fetchEndpoint = useCallback(async (url: string, transform?: (d: any) => any, options?: RequestInit) => {
    ...
    try {
      ...
      console.warn('[KUZGU] Suppressed error:', e instanceof Error ? e.message : e);
  ...
  useEffect(() => {
    ...
    fetchEndpoint('/api/kandilli', d => ({ earthquakes: [...(dataRef.current.earthquakes || []), ...(d.earthquakes || [])] }));
  ...
  useEffect(() => {
    ...
    let delay = 0;
    const staggerFetch = (key: string, fn: () => void) => {
      if ((activeLayers as any)[key] && !layerFetchedRef.current.has(key)) {
        layerFetchedRef.current.add(key); // Mark immediately to prevent double triggers
        setTimeout(fn, delay);
        delay += 600; // Stagger each request by 600ms
      }
    };

    // Flights
    if (activeLayers.flights || activeLayers.military || activeLayers.jets || activeLayers.private) {
      if (!layerFetchedRef.current.has('flights_base')) {
        layerFetchedRef.current.add('flights_base');
        setTimeout(() => fetchEndpoint('/api/flights'), delay);
        delay += 600;
      }
    }

    staggerFetch('satellites', () => fetchEndpoint('/api/satellites?limit=2000', d => ({ satellites: d.satellites })));
    staggerFetch('fires', () => fetchEndpoint('/api/fires'));
    staggerFetch('cctv', () => fetchEndpoint('/api/cctv?region=all&v=2'));
    staggerFetch('radio', () => fetchEndpoint('/api/radio', d => ({ radios: d.radios })));
    staggerFetch('maritime', () => fetchEndpoint('/api/maritime', d => ({ maritime_ports: d.ports, maritime_chokepoints: d.chokepoints, maritime_ships: d.ships })));
    staggerFetch('balloons', () => fetchEndpoint('/api/balloons', d => ({ balloons: d.balloons })));
    staggerFetch('radiation', () => fetchEndpoint('/api/radiation', d => ({ radiation: d.stations })));
    staggerFetch('live_news', () => fetchEndpoint('/api/live-news', d => ({ live_feeds: d.feeds })));
    staggerFetch('telegram_osint', () => fetchEndpoint('/api/telegram-feed?v=2', d => ({ telegram_feeds: d.feeds })));
    staggerFetch('weather', () => fetchEndpoint('/api/weather', d => ({ weather_events: d.events })));
    staggerFetch('infrastructure', () => fetchEndpoint('/api/infrastructure', d => ({ infrastructure: d.infrastructure })));
    staggerFetch('global_incidents', () => fetchEndpoint('/api/gdelt', d => ({ gdelt: d.events })));
    staggerFetch('ankara_social', () => fetchEndpoint('/api/ankara-social', d => ({ ankara_social: d.feeds })));
    staggerFetch('honeypot_targets', () => fetchEndpoint('/api/osint/honeypot/logs', d => ({ honeypot_logs: d.logs })));
    
    // Submarine Cables
    staggerFetch('cables', () => {
      (async () => {
        try {
          const res = await fetch(`/data/submarine-cables.json?v=${Date.now()}`);
          if (res.ok) {
    ...
    staggerFetch('internet_outages', () => fetchEndpoint('/api/ioda', d => ({ outages: d.outages })));
    staggerFetch('malware', () => fetchEndpoint('/api/malware', d => ({ malware: d.events })));
  ...
  useEffect(() => {
    ...
    if (activeLayers.telegram_osint) {
      intervals.push(setInterval(() => fetchEndpoint('/api/telegram-feed', d => ({ telegram_feeds: d.feeds })), 600000)); // 10m
    }
    if (activeLayers.honeypot_targets) {
      intervals.push(setInterval(() => fetchEndpoint('/api/osint/honeypot/logs', d => ({ honeypot_logs: d.logs })), 30000)); // 30s
    }
  ...
  // ── KUZGU SDK — Intelligence Fusion Layer ──
  ...
  return (
    <main className="fixed inset-0 w-full h-full bg-[var(--bg-void)] overflow-hidden">
          ...
          >
            ...
            <div className="w-64 md:w-80 z-[2]">
              ...
              <div className="mt-3 h-4 flex items-center justify-center">
                {[
                  { text: 'GÜVENLİ BAĞLANTI KURULUYOR...', delay: 0.5 },
                  { text: 'AKIŞLAR BAŞLATILIYOR...', delay: 1.1 },
                  { text: 'SENSÖRLER AYARLANIYOR...', delay: 1.7 },
                  { text: 'SİSTEM HAZIR', delay: 2.2 },
                ].map((stage, i) => (
                  <motion.span
                    key={i}
                    initial={{ opacity: 0 }}
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 3 }} className="status-bar-desktop absolute top-4 right-6 z-[200] pointer-events-none flex items-center gap-4 text-[9px] font-mono tracking-widest text-[var(--text-muted)]">
        ...
        <span className="flex items-center gap-1">SİSTEM: <span className={backendStatus === 'connected' ? 'text-[var(--alert-green)]' : 'text-[var(--alert-red)]'}>{backendStatus.toUpperCase()}</span></span>

        {spaceWeather && <span className="hidden lg:inline">GÜNEŞ: <span style={{ color: spaceWeather.storm_color, fontWeight: 700 }}>Kp{spaceWeather.kp_index}</span></span>}

        <span className="hidden lg:inline-flex items-center gap-1">
          <span className="text-[var(--cyan-primary)] font-bold">{Object.values(activeLayers).filter(Boolean).length}</span>
          <span className="text-[var(--text-muted)]/60">AKIŞ</span>
        </span>
          ...
          <AnimatePresence>
            {showIntel && (
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="absolute right-12 top-1/2 -translate-y-1/2 w-80">
                <OsintPanel onSweepVisualize={setSweepData} onGraphPivot={(type, id, label) => {
                  setEntityGraphTarget({ type, id, label });
                  setShowEntityGraph(true);
                ...
                <div className="px-3 pb-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="hud-text text-[9px] text-[var(--text-primary)]">
                      {mobilePanel === 'layers' ? 'LAYERS & STATS' : mobilePanel === 'markets' ? 'MARKETS & INTEL' : mobilePanel === 'intel' ? 'INTEL FEED' : mobilePanel === 'recon' ? 'KUZGU RECON' : 'SEARCH'}
                    </span>
                  ...
                  {mobilePanel === 'recon' && (
                    <div className="space-y-2">
                      <OsintPanel isOpen={true} onClose={() => setMobilePanel(null)} isMobile={true} onSweepVisualize={setSweepData} onGraphPivot={(type, id, label) => { setEntityGraphTarget({ type, id, label }); setShowEntityGraph(true); }} />
                    </div>
                  )}
                </div>
              </motion.div>
      ...
      {/* ── NASA GIBS uydu katmanı bilgi rozeti ── */}
      {activeLayers.gibs && (
        <div className="fixed top-12 left-1/2 -translate-x-1/2 z-[400] px-3 py-1.5 rounded-lg bg-black/85 border border-[#00E5FF]/40 text-[9px] font-mono text-[#00E5FF] backdrop-blur-sm pointer-events-none text-center leading-snug">
          🛰 NASA MODIS · {new Date(Date.now() - 86400000).toISOString().slice(0, 10)} (günlük) · ~250m çözünürlük
          <br />
          <span className="text-[var(--text-muted)]">Bölgesel görüntü — uzaklaşınca görünür. Yakın zoom&apos;daki net görüntü ESRI taban haritasıdır (güncel değil).</span>
        </div>
      )}

      {/* ── AI Analyst (DeepSeek OSINT agent) ── */}
      <AiCommand onMapCommand={handleMapCommand} />
      ...
      <div className="desktop-only absolute bottom-[26px] right-5 z-[200] pointer-events-none text-[6px] font-mono text-[var(--text-muted)]/40 tracking-widest">
        [?] KISAYOLLAR · [F] TAM EKRAN · [S] PAYLAŞ · [R] SIFIRLA
      </div>
