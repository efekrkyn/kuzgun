...
const SECTIONS = [
  { key: 'indices', label: 'ENDEKSLER', icon: LineChart },
  { key: 'stocks', label: 'SAVUNMA', icon: Shield },
  { key: 'oil', label: 'ENERJİ', icon: Droplets },
  { key: 'commodities', label: 'EMTİA', icon: Gem },
  { key: 'crypto', label: 'KRİPTO', icon: Bitcoin },
];
...
export default function MarketsPanel({ data, spaceWeather }: MarketsPanelProps) {
  ...
  const content = (
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6, duration: 0.6 }} className={`glass-panel p-3 pointer-events-auto transition-all duration-300 flex flex-col ${maximized ? 'fixed inset-4 z-[9999] bg-[#0a0a09]/95 backdrop-blur-3xl' : ''}`}>
      <div onClick={() => setExpanded(!expanded)} role="button" tabIndex={0} className="flex items-center justify-between w-full mb-2 cursor-pointer">
        <div className="flex items-center gap-2">
          <BarChart3 className="w-3.5 h-3.5 text-[var(--gold-primary)]" />
          <span className="hud-text text-[12px] text-[var(--text-primary)]">PİYASALAR & İSTİHBARAT</span>
          <span className="gotham-tag gotham-tag--low" style={{ fontSize: '7px', padding: '1px 4px' }}>CANLI</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-1.5 h-1.5 rounded-full bg-[var(--alert-green)] animate-osiris-pulse" />
          <button onClick={(e) => { e.stopPropagation(); setMaximized(!maximized); if (!expanded && !maximized) setExpanded(true); }} className="hover:text-white transition-colors" title={maximized ? "Geri Al" : "Büyüt"}>
          ...
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }}>
              ...
              <div className="mb-2 p-2 rounded-lg border" style={{ borderColor: `${spaceWeather.storm_color}33`, background: `${spaceWeather.storm_color}08` }}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[10px] font-mono tracking-widest text-[var(--text-muted)]">UZAY HAVASI</span>
                  </div>
                ...
                {spaceWeather.solar_flares?.length > 0 && (
                  <div className="mt-1 text-[8px] font-mono text-[var(--text-muted)]">
                    Son patlama: {spaceWeather.solar_flares[0].class}
                  </div>
                )}
              </div>
            )}
            ...
            <div className="space-y-0.5 overflow-y-auto styled-scrollbar mt-2">
              ...
              {(!markets[activeSection] || Object.keys(markets[activeSection]).length === 0) && (
                <div className="text-center py-3 text-[10px] font-mono text-[var(--text-muted)]">{activeSection} yükleniyor...</div>
              )}
            </div>
          </motion.div>
        )}
