...
function OsirisMap({ data, activeLayers, onEntityClick, onMouseCoords, onRightClick, onViewStateChange, flyToLocation, projection = 'globe', mapStyle = 'dark', sweepData, scanTargets = [], demoMode = false }: OsirisMapProps) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      createDot(map, 'dot-radio', '#FFEB3B', 12);

      // Sources
      const sources = ['flights','military','jets','private-fl','satellites','earthquakes','gdelt','gps-jamming','day-night','cctv','radio','fires','weather','infrastructure','maritime','maritime-choke','maritime-ships','live-news','sigint-news','telegram-osint','ankara-social','conflict-zones', 'war-alerts-targets', 'war-alerts-lines', 'balloons', 'radiation', 'ip-sweep-devices', 'ip-sweep-pulse', 'ip-sweep-connections', 'scan-targets', 'sdk-entities', 'sdk-links', 'ioda-outages', 'malware-nodes', 'honeypot-targets'];
      sources.forEach(s => map.addSource(s, { type: 'geojson', data: EMPTY_FC }));

      // NASA GIBS — günlük (near-real-time) gerçek uydu görüntüsü (MODIS Terra True Color)
      try {
        const gibsDate = new Date(Date.now() - 86400000).toISOString().slice(0, 10);
        map.addSource('gibs', {
          type: 'raster',
      ...
      } catch { /* gibs eklenemezse sessizce geç */ }

      // Google Traffic Overlay (Roads + Traffic, Transparent)
      map.addSource('google-traffic-src', {
        type: 'raster',
        tiles: ['https://mt1.google.com/vt/lyrs=h,traffic&hl=tr&x={x}&y={y}&z={z}'],
        tileSize: 256,
      });
      map.addLayer({ id: 'google-traffic', type: 'raster', source: 'google-traffic-src', layout: { visibility: 'none' }, paint: { 'raster-opacity': 0.8 } });
      ...
      // ANKARA SOCIAL
      map.addLayer({
        id: 'layer-ankara-social', type: 'circle', source: 'ankara-social',
        paint: { 'circle-color': '#E1306C', 'circle-radius': 8, 'circle-stroke-width': 2, 'circle-stroke-color': '#000000', 'circle-opacity': 0.9, 'circle-blur': 0.1 }
      });
      ...
      map.addLayer({ id: 'radio-glow', type: 'circle', source: 'radio', paint: {
        'circle-radius': ['interpolate',['linear'],['zoom'], 1,12, 5,24],
        'circle-color': '#FFEB3B', 'circle-opacity': 0.15, 'circle-blur': 1,
      }});
      map.addLayer({ id: 'radio-dots', type: 'symbol', source: 'radio', layout: {
        'icon-image': 'dot-radio', 'icon-allow-overlap': true,
      }});
      map.addLayer({ id: 'radio-label', type: 'symbol', source: 'radio', minzoom: 6, layout: {
        'text-field': ['get','name'], 'text-size': 10, 'text-font': ['Open Sans Bold'],
        'text-offset': [0, 1.5], 'text-max-width': 14, 'text-allow-overlap': false,
      }, paint: { 'text-color': '#FFEB3B', 'text-halo-color': '#000', 'text-halo-width': 1 }});
      ...
      // Telegram OSINT - cyan terminal dots
      map.addLayer({ id: 'telegram-glow', type: 'circle', source: 'telegram-osint', paint: {
        'circle-radius': ['interpolate',['linear'],['zoom'], 1,8, 5,14, 10,22],
        'circle-color': '#00FFFF', 'circle-opacity': 0.1, 'circle-blur': 1,
      }});
      map.addLayer({ id: 'telegram-dots', type: 'circle', source: 'telegram-osint', paint: {
        'circle-radius': ['interpolate',['linear'],['zoom'], 1,4, 5,6, 10,10],
        'circle-color': '#00FFFF', 'circle-opacity': 0.85,
        'circle-stroke-width': 2, 'circle-stroke-color': '#00FFFF', 'circle-stroke-opacity': 0.5,
      }});
      map.addLayer({ id: 'telegram-label', type: 'symbol', source: 'telegram-osint', minzoom: 4, layout: {
        'text-field': ['get','title'], 'text-size': 9, 'text-font': ['Open Sans Regular'],
        'text-offset': [0, 1.8], 'text-max-width': 12, 'text-allow-overlap': false,
      }, paint: { 'text-color': '#00FFFF', 'text-halo-color': '#000', 'text-halo-width': 1, 'text-opacity': 0.8 }});

      // HONEYPOT TARGETS - glowing red blips
      map.addLayer({ id: 'honeypot-targets-glow', type: 'circle', source: 'honeypot-targets', paint: {
        'circle-radius': ['interpolate',['linear'],['zoom'], 1,12, 5,20, 10,35],
        'circle-color': '#FF0033', 'circle-opacity': 0.3, 'circle-blur': 1,
      }});
      map.addLayer({ id: 'honeypot-targets-dots', type: 'circle', source: 'honeypot-targets', paint: {
        'circle-radius': ['interpolate',['linear'],['zoom'], 1,5, 5,8, 10,12],
        'circle-color': '#FF0033', 'circle-opacity': 0.9,
        'circle-stroke-width': 2, 'circle-stroke-color': '#FFFFFF', 'circle-stroke-opacity': 0.8,
      }});
      map.addLayer({ id: 'honeypot-targets-label', type: 'symbol', source: 'honeypot-targets', minzoom: 4, layout: {
        'text-field': ['get','ip_address'], 'text-size': 10, 'text-font': ['Open Sans Bold'],
        'text-offset': [0, 2], 'text-max-width': 15, 'text-allow-overlap': false,
      }, paint: { 'text-color': '#FF0033', 'text-halo-color': '#000', 'text-halo-width': 2, 'text-opacity': 0.9 }});
      ...
      // ══ KUZGU SDK — Lattice Intelligence Mesh ══
      ...
      map.on('click', layer, e => {
        ...
        popup(coords, `<div style="${pStyle}border:1px solid rgba(212,175,55,0.3);">
          ...
          <div style="margin-top:12px;display:flex;gap:6px;flex-wrap:wrap;">
            <button onclick="window.dispatchEvent(new CustomEvent('ai-command', { detail: 'Uçuş ${cs} (Model: ${p.model||'Bilinmiyor'}) hakkında açık kaynak istihbaratı topla' }))" style="background:#D4AF3720;border:1px solid #D4AF3780;color:#D4AF37;padding:4px 8px;border-radius:4px;font-size:9px;cursor:pointer;font-weight:bold;">🤖 AI DOSYASI</button>
    ...
    map.on('click', 'telegram-dots', e => {
      ...
      const p = e.features[0].properties as any;
      const coords = (e.features[0].geometry as any).coordinates;
      popup(coords, `<div style="${pStyle}border:1px solid rgba(0,255,255,0.3);">
        <div style="color:#00FFFF;font-size:12px;font-weight:700;margin-bottom:6px;">💬 TELEGRAM OSINT</div>
        <div style="font-size:9px;color:#E8E6E0;margin-bottom:8px;line-height:1.4;">${p.title||'Intel Report'}</div>
        <div style="display:grid;grid-template-columns:1fr;gap:4px;font-size:9px;margin-bottom:8px;">
          <div><span style="color:#5C5A54;">SOURCE</span><br/><span style="color:#00E5FF;">${p.source}</span></div>
          <div><span style="color:#5C5A54;">PUB DATE</span><br/><span style="color:#E8E6E0;">${p.published ? new Date(p.published).toLocaleString() : ''}</span></div>
        </div>
        <div style="display:flex;gap:6px;">
          ${p.link ? `<a href="${p.link}" target="_blank" style="${linkStyle}color:#00FFFF;border:1px solid rgba(0,255,255,0.4);background:rgba(0,255,255,0.1);">OPEN SOURCE</a>` : ''}
        </div>
      </div>`);
    });

    // ── HONEYPOT TARGETS ──
    map.on('click', 'honeypot-targets-dots', e => {
      if (!e.features?.length) return;
      const p = e.features[0].properties as any;
      const coords = (e.features[0].geometry as any).coordinates;
      popup(coords, `<div style="${pStyle}border:1px solid rgba(255,0,51,0.5);">
        <div style="color:#FF0033;font-size:12px;font-weight:800;margin-bottom:6px;letter-spacing:1px;">🚨 TARGET ACQUIRED</div>
        <div style="font-size:11px;color:#FFF;margin-bottom:8px;font-family:monospace;">IP: ${p.ip_address}</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;font-size:9px;margin-bottom:8px;">
          <div><span style="color:#FF0033;">DEVICE</span><br/><span style="color:#E8E6E0;">${p.device_type}</span></div>
          <div><span style="color:#FF0033;">LOCATION</span><br/><span style="color:#E8E6E0;">${p.city}, ${p.country}</span></div>
        </div>
        <div style="font-size:8px;color:#888;word-break:break-all;margin-top:4px;">UA: ${p.user_agent}</div>
        <div style="font-size:8px;color:#888;margin-top:4px;">TIME: ${p.captured_at ? new Date(p.captured_at).toLocaleString() : ''}</div>
      </div>`);
    });

    // ── ANKARA SOCIAL ──
    map.on('click', 'layer-ankara-social', e => {
      if (!e.features?.length) return;
      const p = e.features[0].properties as any;
      const coords = (e.features[0].geometry as any).coordinates;
      popup(coords, `<div style="${pStyle}border:1px solid rgba(225,48,108,0.3);">
        <div style="color:#E1306C;font-size:12px;font-weight:700;margin-bottom:6px;">👥 ANKARA SOSYAL AĞ</div>
        <div style="font-size:9px;color:#E8E6E0;margin-bottom:8px;line-height:1.4;">${p.description||'Social Post'}</div>
        <div style="display:grid;grid-template-columns:1fr;gap:4px;font-size:9px;margin-bottom:8px;">
          <div><span style="color:#5C5A54;">AUTHOR</span><br/><span style="color:#E1306C;">${p.title}</span></div>
          <div><span style="color:#5C5A54;">SOURCE</span><br/><span style="color:#E8E6E0;">${p.source}</span></div>
        </div>
        <div style="display:flex;gap:6px;">
          ${p.link ? `<a href="${p.link}" target="_blank" style="${linkStyle}color:#E1306C;border:1px solid rgba(225,48,108,0.4);background:rgba(225,48,108,0.1);">GÖRÜNTÜLE</a>` : ''}
        </div>
      </div>`);
    });
    ...
    map.on('click', 'sigint-news-dots', e => {
      ...
      const coords = (e.features[0].geometry as any).coordinates;
      popup(coords, `<div style="${pStyle}border:1px solid rgba(212,175,55,0.3);">
        <div style="color:#D4AF37;font-size:12px;font-weight:700;margin-bottom:6px;">📰 SIGINT REPORT</div>
        <div style="font-size:9px;color:#E8E6E0;margin-bottom:8px;line-height:1.4;">${p.title||'Intelligence Brief'}</div>
        <div style="display:grid;grid-template-columns:1fr;gap:4px;font-size:9px;margin-bottom:8px;">
          <div><span style="color:#5C5A54;">SOURCE</span><br/><span style="color:#D4AF37;">${p.source}</span></div>
          <div><span style="color:#5C5A54;">PUB DATE</span><br/><span style="color:#E8E6E0;">${p.published ? new Date(p.published).toLocaleString() : ''}</span></div>
        </div>
        <div style="display:flex;gap:6px;">
          ${p.link ? `<a href="${p.link}" target="_blank" style="${linkStyle}color:#D4AF37;border:1px solid rgba(212,175,55,0.4);background:rgba(212,175,55,0.1);">READ FULL</a>` : ''}
        </div>
      </div>`);
    });
      ...
      map.on('click', layer, e => {
        ...
        popup([coords.lng, coords.lat], `<div style="${pStyle}border:1px solid ${domainColor}40;">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:9px;margin-bottom:8px;">
            ...
            <div><span style="color:#5C5A54;">SOURCE</span><br/><a href="${srcUrl}" target="_blank" style="color:${domainColor};text-decoration:underline;cursor:pointer;">${p.source || 'KUZGU'}</a></div>
          </div>
    ...
    ['radio-dots','conflict-icons','cctv-dots','eq-circles','sat-dots','fires-heat','gdelt-dots','weather-dots','infra-dots','maritime-dots','choke-dots','news-dots','telegram-dots','sigint-news-dots','balloon-dots','rad-dots','ship-dots','sweep-device-dots','scan-targets-dots','sdk-sea','sdk-sea-glow','sdk-sea-atmo','sdk-air','sdk-air-glow','sdk-air-atmo','sdk-intel','sdk-intel-glow','sdk-intel-atmo'].forEach(layer => {
      map.on('mouseenter', layer, () => { map.getCanvas().style.cursor = 'pointer'; });
      map.on('mouseleave', layer, () => { map.getCanvas().style.cursor = ''; });
    });
    ...
    map.on('click', 'radio-dots', e => {
      ...
      const coords = (e.features[0].geometry as any).coordinates;
      const html = `
        <div style="padding:4px;width:240px;pointer-events:auto;">
          <div style="font-family:'Orbitron',sans-serif;font-size:12px;color:#FFEB3B;margin-bottom:8px;display:flex;align-items:center;gap:6px;">
            <span style="display:inline-block;width:8px;height:8px;background:#FFEB3B;border-radius:50%;box-shadow:0 0 8px #FFEB3B;animation:pulse 1.5s infinite;"></span>
            ${p.name}
          </div>
          <div style="font-size:11px;color:#888;margin-bottom:12px;font-family:monospace;">
            ${p.city}, ${p.country} • ${p.category}<br/>
            SOURCE: <span style="color:#FFF;">${p.source}</span>
          </div>
          <div style="background:#000;border:1px solid #333;border-radius:4px;padding:8px;margin-bottom:4px;">
            <audio controls autoplay name="media" style="width:100%;height:30px;outline:none;">
              <source src="${p.stream_url}" type="audio/mpeg">
            </audio>
          </div>
          <style>
      ...
      `;
      new maplibregl.Popup({ className: 'kuzgu-popup', closeButton: true, maxWidth: '300px' })
    ...
    map.on('click', 'ship-dots', e => {
      ...
      popup(coords, `<div style="${pStyle}border:1px solid ${color}40;">
        ...
        <div style="margin-top:8px;">
          <button onclick="window.dispatchEvent(new CustomEvent('ai-command', { detail: 'Gemi ${p.name} (Bayrak: ${p.flag}, Tür: ${p.type}) hakkında açık kaynak istihbaratı topla' }))" style="background:#D4AF3720;border:1px solid #D4AF3780;color:#D4AF37;padding:4px 8px;border-radius:4px;font-size:9px;cursor:pointer;font-weight:bold;width:100%;">🤖 AI DOSYASI OLUŞTUR</button>
        </div>
      </div>`);
  ...
  // NASA GIBS uydu katmanı görünürlüğü
  useEffect(() => {
    if (!mapReady || !mapRef.current) return;
    const map = mapRef.current;
    if (map.getLayer('gibs-layer')) {
      map.setLayoutProperty('gibs-layer', 'visibility', activeLayers.gibs ? 'visible' : 'none');
    }
  }, [mapReady, activeLayers.gibs]);
  ...
  useEffect(() => {
    ...
    setGeo('private-fl', activeLayers.private ? toFeatures(data.private_flights, 1) : []);
    setGeo('jets', activeLayers.jets ? toFeatures(data.private_jets, 1) : []);
    setGeo('military', activeLayers.military ? toFeatures(data.military_flights, 1) : []);
  ...
  useEffect(() => {
    if (!mapReady) return;
    setGeo('radio', activeLayers.radio && data.radios ? data.radios.map((r: any) => ({ type: 'Feature', geometry: { type: 'Point', coordinates: [r.lng, r.lat] }, properties: r })) : []);
  }, [mapReady, data.radios, activeLayers.radio, setGeo]);
  ...
  // ══ KUZGU SDK — Lattice Sensor Mesh ══
  // Uses real submarine cable data for SEA domain, curated routes for AIR/INTEL
  useEffect(() => {
    if (!mapReady) return;
    let defenseNodes: any[] = [];
    if (activeLayers.sdk_naval) { // Use SDK Naval toggle for Defense nodes for now
      const ankaraNodes = [
        { id: 'TUSAS', name: 'TUSAŞ (TAI)', lat: 40.0886, lng: 32.5694, type: 'AEROSPACE', color: '#00E5FF' },
        { id: 'ASELSAN_MAC', name: 'ASELSAN Macunköy', lat: 39.9675, lng: 32.7661, type: 'DEFENSE ELECTRONICS', color: '#00E5FF' },
        { id: 'ASELSAN_GOL', name: 'ASELSAN Gölbaşı', lat: 39.7915, lng: 32.8105, type: 'RADAR/EW', color: '#00E5FF' },
        { id: 'HAVELSAN', name: 'HAVELSAN', lat: 39.9077, lng: 32.7766, type: 'C4ISR / CYBER', color: '#00E5FF' },
        { id: 'ROKETSAN', name: 'ROKETSAN', lat: 39.9436, lng: 33.2422, type: 'MISSILE SYSTEMS', color: '#00E5FF' }
      ];
      defenseNodes = ankaraNodes.map(n => ({
        type: 'Feature',
        geometry: { type: 'Point', coordinates: [n.lng, n.lat] },
        properties: { id: n.id, name: n.name, domain: 'DEFENSE', color: n.color, type: n.type }
      }));
    }
    setGeo('sdk-entities', defenseNodes);
    ...
    if (activeLayers.sdk_naval) {
      const ankaraNodes = [
        { id: 'TUSAS', lat: 40.0886, lng: 32.5694 },
        { id: 'ASELSAN_MAC', lat: 39.9675, lng: 32.7661 },
        { id: 'ASELSAN_GOL', lat: 39.7915, lng: 32.8105 },
        { id: 'HAVELSAN', lat: 39.9077, lng: 32.7766 },
        { id: 'ROKETSAN', lat: 39.9436, lng: 33.2422 }
      ];
      // Connect HAVELSAN (C4ISR) to all others
      const c4isr = ankaraNodes.find(n => n.id === 'HAVELSAN')!;
      for (const node of ankaraNodes) {
        if (node.id === 'HAVELSAN') continue;
        links.push({
          type: 'Feature',
          geometry: { type: 'LineString', coordinates: [[c4isr.lng, c4isr.lat], [node.lng, node.lat]] },
          properties: { domain: 'DEFENSE_LINK', source: 'C4ISR_NETWORK', color: '#00E5FF' }
        });
      }
    }
  ...
  useEffect(() => {
    if (!mapReady) return;
    const items = data.telegram_feeds || [];
    setGeo('telegram-osint', activeLayers.telegram_osint && items.length > 0
      ? items.map((n: any) => ({
          type: 'Feature',
          geometry: { type: 'Point', coordinates: [n.lng, n.lat] },
          properties: { title: n.title, source: n.source, link: n.link, published: n.published }
        }))
      : []);
  }, [mapReady, data.telegram_feeds, activeLayers.telegram_osint, setGeo]);

  useEffect(() => {
    if (!mapReady) return;
    const items = data.honeypot_logs || [];
    setGeo('honeypot-targets', activeLayers.honeypot_targets && items.length > 0
      ? items.map((n: any) => ({
          type: 'Feature',
          geometry: { type: 'Point', coordinates: [n.lng, n.lat] },
          properties: { ip_address: n.ip_address, user_agent: n.user_agent, device_type: n.device_type, city: n.city, country: n.country, captured_at: n.captured_at }
        }))
      : []);
  }, [mapReady, data.honeypot_logs, activeLayers.honeypot_targets, setGeo]);

  useEffect(() => {
    if (!mapReady) return;
    const items = data.ankara_social || [];
    setGeo('ankara-social', activeLayers.ankara_social && items.length > 0
      ? items.map((n: any) => ({
          type: 'Feature',
          geometry: { type: 'Point', coordinates: [n.lng, n.lat] },
          properties: { 
            ...
            link: n.url, 
            published: n.time, 
            source: n.platform === 'twitter' ? 'X / Twitter' : 'Instagram' 
          }
        }))
      : []);
  }, [mapReady, data.ankara_social, activeLayers.ankara_social, setGeo]);
  ...
  useEffect(() => {
    ...
    setVis(['radio-glow','radio-dots','radio-label'], activeLayers.radio);
    ...
    setVis(['telegram-glow','telegram-dots','telegram-label'], activeLayers.telegram_osint);
    setVis(['layer-ankara-social'], activeLayers.ankara_social);
    setVis(['google-traffic'], activeLayers.google_traffic);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (projection === 'globe') {
        ...
        } catch (e) { console.warn('[KUZGU] Suppressed error:', e instanceof Error ? e.message : e); }
      } else {
